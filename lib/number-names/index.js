module.exports = number => {
  ...
  if (numberAsString.length <= 3) {
    if (numberAsString.length === 1) {
      const single = returnSingle(numberAsString.split('')[0])
      return `${single}`
    }

    if (numberAsString.length === 2) {
      if (numberAsString.split('')[0] === '1') {
        const doubleDigitNumber = numberAsString.split('')[0] + numberAsString.split('')[1]
        return returnTeens(doubleDigitNumber, 10)
      }
      if (numberAsString.split('')[1] === '0') {
        return tens(numberAsString.split('')[0])
      }
      const dec = tens(numberAsString.split('')[0], 10)
      const single = returnSingle(numberAsString.split('')[1])
      return `${dec} ${single}`
    }
    if (numberAsString.length === 3) {
      return 'hundred'
