module.exports = number => {
  ...
  if (number.toString().length <= 3) {
    ...
    if (number.toString().length === 2) {
      if (parseInt(number.toString().split('')[0], 10) === 1) {
        const doubleDigitNumber = number.toString().split('')[0] + number.toString().split('')[1]
        return returnTeens(parseInt(doubleDigitNumber, 10))
      }
