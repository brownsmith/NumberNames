module.exports = number => {
  ...
  if (numberAsString.length > 3 && numberAsString.length <= 6) {
    ...
    if (numberAsString.length === 4) {
      const number1 = returnSingle(numberAsString[0])
      const theRest = getHundreds(numberAsString[1] + numberAsString[2] + numberAsString[3])
      return `${number1} thousand ${theRest}`
    }
    if (numberAsString.length === 5) {
      const tensOfThousands = doubleNumber(numberAsString[0] + numberAsString[1])
      const theRest = getHundreds(numberAsString[2] + numberAsString[3] + numberAsString[4])
      return `${tensOfThousands} thousand ${theRest}`
    }
    if (numberAsString.length === 6) {
      const hundredsOfThousands = getHundreds(
        numberAsString[0] + numberAsString[1] + numberAsString[2],
      )
      const theRest = getHundreds(numberAsString[3] + numberAsString[4] + numberAsString[5])
      return `${hundredsOfThousands} thousand ${theRest}`
    }
